...
class my_env extends uvm_env;
    ...
    //----------INTERFACE
    //
    virtual interface apb_if apb0_vif, apb1_vif;
    string header = "";
    //
    apb_coverage cov_h[];
	apb_uart_scoreboard scb;
	apb_agent apb_agt_h[];
	uart_agent uart_agt_h[];
	env_config env_cfg_h;
	virtual_sequencer vsqr_h;
	//reg model
	uvm_reg_predictor #(apb_seq_item) predictor_h[];//, predictor_h1;
	uart_reg_model reg_model_h[];//, reg_model_h1;
	reg_apb_adapter adapter_h[];//, adapter_h1;		
	uvm_reg_backdoor reg_bd_h[];
    //reg callbacks' handle
    ctrl_reg_cbs ctrl_cbs_h; 
    status_reg_cbs status_cbs_h; 
    div_reg_cbs div_cbs_h; 
	//apb config
...
endclass
	...
	function void my_env::build_phase(uvm_phase phase);
		...
		if(!uvm_config_db#(env_config)::get(this, "", "env_cfg", env_cfg_h)) begin
		...
		else begin: ENV_CFG_FOUND
			set_up_done = 0;
			cov_h = new[env_cfg_h.no_of_agent];
			...
			foreach(apb_agt_h[i]) begin
				cov_h[i] = apb_coverage::type_id::create($sformatf("cov_h%0d", i), this);	
	...
	if(set_up_done) begin 
		//if(env_cfg_h.apb_uart_scoreboard == 1) begin
			scb = apb_uart_scoreboard::type_id::create("scb", this);	
		//end	
        ...
        //-----------------------APB AGENT Settings
        //=============================================================================
		foreach(apb_agt_h[i]) begin: APB_AGENT_SETUP
        //
        //retrieve interface
        //
			if(!uvm_config_db#(virtual interface apb_if)
				::get(this, "", $sformatf("apb%0d_if", i), apb_cfg_h[i].vif)) begin
				`uvm_fatal(get_type_name(), $sformatf("APB[%0d] VIF not FOUND!!!", i))
			end
			//
			//store config after retrieving INTERFACE
            //
		    ...
		    predictor_h[i] = uvm_reg_predictor#(apb_seq_item)::type_id::create(
    ...
    //=============================================================================
    //-----------------------Add CALLBACKS for all registers
    //=============================================================================
     ctrl_cbs_h = ctrl_reg_cbs::type_id::create("ctrl_cbs_h");
     status_cbs_h = status_reg_cbs::type_id::create("status_cbs_h");
     div_cbs_h = div_reg_cbs::type_id::create("div_cbs_h");
endfunction
	//
	...
	//	apb_agt_h[i].apb_mon_h.pseltb_ap.connect(scb.pseltb_fifo[i].analysis_export);
		apb_agt_h[i].apb_mon_h.apb_item_ap.connect(scb.apb_txn_fifo[i].analysis_export);
		apb_agt_h[i].apb_mon_h.Other_ap.connect(scb.apb_to_slv_fifo[i].analysis_export);
        //
		apb_agt_h[i].apb_mon_h.apb_item_ap.connect(cov_h[i].analysis_export);
		`uvm_info(get_type_name(), $sformatf("[APB%0d]SUCCEED TO CONNECT MON_SCB ports!!!",i), UVM_HIGH)
		//
        ...
        `ifdef INTERRUPT
		uart_agt_h[i].uart_mon_h.int_ap.connect(scb.int_fifo[i].analysis_export);
        `endif
		`uvm_info(get_type_name(), $sformatf("[UART%0d]SUCCEED TO CONNECT MON_SCB ports!!!",i), UVM_HIGH)
		//
		if(vseqr_existing) begin
		...
		//
            ...
            `uvm_info(get_type_name(), $sformatf("[REG_MODEL%0d]SUCCEED TO SET SEQUENCER!!!",i), UVM_HIGH)
        ...
        //----set regs callbacks
        //
        uvm_callbacks#(uvm_reg, ctrl_reg_cbs)::add(reg_model_h[i].CTRL, ctrl_cbs_h);
        uvm_callbacks#(uvm_reg, status_reg_cbs)::add(reg_model_h[i].STATUS, status_cbs_h);
        uvm_callbacks#(uvm_reg, div_reg_cbs)::add(reg_model_h[i].DIV, div_cbs_h);
		//
	    end //end of FOR_LOOP
		//
		set_up_done = 0;
