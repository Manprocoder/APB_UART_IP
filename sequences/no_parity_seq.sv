...
//--File name: no_parity_seq.sv
//--Author: Nguyen Ngoc Man
//--Description: check UART operation with no parity 
//--Configuration of this class is implemented in virtual_no_parity_seq class
//======================================================================================
import apb_pkg::*;
import uart_ral_pkg::*;
import env_pkg::*;
//
class no_parity_seq#(SYS_CLK = 50_000_000, OVERSAMPLE = 16, BAUD_RATE=9600) extends uart_base_seq;
typedef no_parity_seq#(SYS_CLK, OVERSAMPLE, BAUD_RATE) seq_name;
	parameter DIVISOR = SYS_CLK/(OVERSAMPLE * BAUD_RATE) - 1;
    parameter CLK_CYCLE = (10**9/SYS_CLK);
	`uvm_object_param_utils(seq_name)
	//
	int i;
	//
	function new(string name = "no_parity_seq");
		super.new(name);
        i = 0;
	endfunction
	//
	...
	virtual task body();
		...
		assert(rm_h.CTRL.randomize() with {
			rm_h.CTRL.parity_case == NO_PARITY;
		...
		if(rx_enable) begin: OPERATOR_OF_RX 
            #(DIVISOR*OVERSAMPLE*CLK_CYCLE*10*8);
